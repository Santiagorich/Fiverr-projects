        ...
        if (data.length) {
          ...
          $.each(data, function (i, item) {
            ...
            if (!empty(item.item.alt_name_1)) {
              ...
              $("#menu-bodys #product-" + item.item.id + " .counter em").text(
                num
              );
              $("#menu-bodys #product-" + item.item.id + " .counter").css(
                "pointer-events",
                "none"
              );
              ...
              //カウンターのクリックを無効化
              $("#menu-bodys #product-" + id + " .counter").css(
                "pointer-events",
                "none"
              );
...
function setTakeout() {
  ...
  window.doCheckin = function () {
    ...
    if (ExternalInterface.checkin.menu != $emenu.menu_mode) {
      //再起動
      ExternalInterface.doCheckin(
        $takeout_table_no,
        "",
        "1",
        "0",
        "000",
        ExternalInterface.checkin.menu,
        "    ",
        "null",
        "0",
        "0"
      ...
      $("#staff-call")._click(function () {
        $emenu.message.confirm("staffcall_confirm", function () {
          var item = {
            ...
            code: Config.staffcall.id,
          }; //scope.menudata.menumst[ Config.staffcall.id ];
          if (empty(item)) {
          ...
          var slip_no = tbn;
          $emenu.order.setOrderRequest(
            slip_no,
            item_data,
            $emenu.staff_call.loaded,
            $emenu.staff_call.onError,
            Config.orderdate_insert
          );
  ...
  $(document).bind("ORDER_COMPLETE", function () {
    ...
    if (!$emenu.payment.enable) {
      $emenu.order_complete_timer = $.timer(
        function () {
          //$("#order-complete").hide();
        ...
        Config.order_complete.close_time,
        true
      );
    }
  });
  ...
  $emenu.payment.createQR = function () {
    $("#payment-qr-container").html(
      '<h1 class="' +
        Config.message.text_class +
        '">' +
        $emenu.alternate.getString("payment-lo0ading") +
        "</h1>"
    );
...
$(document).bind("SELECT_SHOW", function () {
  ...
  $("#select .stepbox .select-btn").each(function () {
    try {
      var data = $emenu.select.getSelectItem(
        $(this).parents(".stepbox").data("index"),
        $(this).data("id")
      );
...
$(document).bind("DETAIL_SHOW", function () {
  ...
  if (group_data.length) {
    ...
    var a = btn
      .clone(false)
      .insertAfter(btn)
      .addClass("custom custom1")
      .css("margin-right", "15px");
    if (empty(detail_data.name_2)) {
      a.html(
        String(
          detail_data.name_1 + "" + detail_data.name_2 + detail_data.name_3
        ) +
          "<br><span>" +
          priceText(detail_data.price) +
          "</span>"
      );
    } else {
      a.html(
        String(
          detail_data.name_1 + "<br>" + detail_data.name_2 + detail_data.name_3
        ) +
          "<br><span>" +
          priceText(detail_data.price) +
          "</span>"
    ...
    $.each(group_data, function (i, gr) {
      ...
      if (empty(gr.name_2)) {
        b.html(
          String(gr.name_1 + "" + gr.name_2 + gr.name_3) +
            "<br><span>" +
            priceText(gr.price) +
            "</span>"
        );
        ...
        b.html(
          String(gr.name_1 + "<br>" + gr.name_2 + gr.name_3) +
            "<br><span>" +
            priceText(gr.price) +
            "</span>"
        );
...
$(document).bind("MESSAGE_SHOW", function () {
  ...
  if ($("#message").hasClass("6100")) {
    ...
    $("#message").hasClass("order_complete") ||
    $("#message").hasClass("order_complete2")
  ) {
    $emenu.payment.show();
